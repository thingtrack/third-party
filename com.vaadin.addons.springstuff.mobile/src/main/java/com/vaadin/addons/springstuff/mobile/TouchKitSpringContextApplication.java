package com.vaadin.addons.springstuff.mobile;
...
/**
 * TouchKitSpringContextApplication is a specialized {@link Application} implementation to
 * help building Vaadin applications designed specifically for various touch
 * devices and each application instance is given its own Spring application context. 
 * It provides quick access to the {@link WebBrowser} object via
...
public abstract class TouchKitSpringContextApplication extends
		SpringContextApplication {

	private boolean browserDetailsReady = false;

    private static ThreadLocal<TouchKitSpringContextApplication> activeApplication = new ThreadLocal<TouchKitSpringContextApplication>();
    
	
    public TouchKitSpringContextApplication(){
    	super();
    ...
    public static TouchKitSpringContextApplication get() {
     ...
     * window.
     */
    public abstract void onBrowserDetailsReady(ConfigurableWebApplicationContext context);
    ...
    protected void doOnRequestStart(HttpServletRequest request,
         ...
         if (!browserDetailsReady) {
             if (request.getParameter("repaintAll") != null) {
                 ...
                 if(isRunning()) {
                 	onBrowserDetailsReady(getApplicationContext());
                 }
             }
         }
    }
